/* ============================================================
   ...
   Concepts: GROUP BY, HAVING, DATEDIFF, CASE WHEN, SELECT INTO,
             ...
             Correlated Subqueries, Double CTE, ISNULL/COALESCE,
             YEAR/MONTH/DATEPART, UNION ALL, SQL Views
   ============================================================ */
...
-- Assign a unique row number to employees within each
--     department by salary using ROW_NUMBER with PARTITION BY
SELECT
    Employee_Name,
    Department,
    Salary,
...
-- Extract the joining quarter for each employee using DATEPART
SELECT
    Employee_Name,
...
-- Reusable View: Department-wise average salary (SQL View)
CREATE or ALTER VIEW Department_AVG_Salary AS
SELECT
    Department,
    AVG(Salary) AS average_salary
FROM
